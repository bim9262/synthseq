...
/**
 * Prefix tree. Stores characters in a tree. The path from the root to a
 * terminating node makes up a word.
 *
 * The tree is implemented with stack-like traversal methods to facilitate its
 * use with the recursive solving algorithm.
 *
 * Keeping tree state and the recursive grid solving algorithm in lock step
 * is dramatically faster than performing a lookup in a traditional collection
 * at every step.
 *
...
public class Trie {
	...
	public ArrayList<String> getWords(String start){
    ...
    /**
     * Adds a word to the Trie (prefix tree) Each letter in the word is a node
     * in the Trie; each letter is the parent of the next letter.
     *
     * @param add
     ...
     *
     * @param c
     *            the character that is checked for existence in the current
     *            node's branches
     ...
     *
     * @param c
     *            The next character that the node will point to
     ...
     * current Node is a (full) word. This is indicated by the existance of a
     * '{' character Node as one of the children of the current Node.
     *
